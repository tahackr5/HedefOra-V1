...
| R-013 | Aktif checkout'un OneDrive altında olması sync/lock/performans ve tek-makine kurtarma riski yaratır | Medium | High | W000 writer worktree'lerini OneDrive dışında tut; owner onayıyla daha sonra Dev Drive veya `C:\Projeler\HedefOra` altına temiz clone taşı; GitHub + ikinci şifreli off-site restore testi tamamla | Owner/Orchestrator | MITIGATING |
| R-014 | GitHub merge yöntemi veya main koruması SHA-mühürlü ownership kanıtını yeniden yazar ya da bypass eder | Medium | Critical | Yalnız two-parent merge commit; squash/rebase/linear history/force push/delete kapalı; required PR checks; final main merge-wrapper gate'i | Owner/Orchestrator | OPEN |
