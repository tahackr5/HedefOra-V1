...
- Tek writer orchestrator; branch `codex/w001-t04f-postmerge-codeql5`, worktree `T04F-CODEQL5`. Source ownership yalnız `tests/integration/postgres/live/runner.test.mjs`; shared evidence ownership yalnız bu promotion'da gerçekten değiştirilen `state/ACTIVE-WAVE.md`, `state/RELEASE-LEDGER.md`, `state/W001-EVIDENCE.md`, `state/W001-OWNERSHIP.md`, `state/W001-PG17-FIXTURE.md`, `state/W001-PHASE-B.md`. Security ve cold reviewer read-only'dir. Production/deployment/SSH/DNS/secret kapsam dışıdır.
- Living manifest zinciri kesintisiz ve dar tutulur: `e8335c35f27f0bdf3a2470f65d73468e75791283 → 799630bdad8cde8c784bc3a797731d9186bd96e0` yalnız JSON seal; `799630bdad8cde8c784bc3a797731d9186bd96e0 → df67eb1d2c59009552602a666f352605494587ce` owner merge-wrapper ve first-parent `aba3d13 → 799630bd` exact 90-path delta; `df67eb1 → 4d62c24` yalnız runner test; `4d62c24 → E1` yalnız yukarıdaki altı state yolu. Final `E1 → S1` yalnız `state/W001-OWNERSHIP.json` trailing seal'idir. Endpoint diff'in boş olması gerçek merge-wrapper delta'sını silmez.
- Acceptance: exact S1 ownership/full-tree/pinned Go/local R-016; fresh security ve cold review; hosted CI/trusted R-016/Dependency Review/CodeQL; açık HIGH finding0; ardından yalnız S1'i kapsayan yeni owner exact-head merge onayı. PR #8'in `799630bd` onayı bu remediation head'ine taşınmaz. T04G ancak yeni owner merge ve post-merge full security checkpoint sonrasında açılır.
