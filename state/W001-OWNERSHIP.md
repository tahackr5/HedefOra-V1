...
Task-phase base `7a1e124e432b51694e7d60c0d3d1589867a8835f` / tree `ec6d12847c657adedbd84e44565598d234c7b928`; historical wave start değişmez. Tek writer orchestrator, branch `codex/w001-r016-go-graph`, worktree `T04FB-CONTROL`. Architecture implementation proposal-only, security/cold read-only. Yedi kod/sözleşme yolu: `scripts/supply-chain/{run,run.test,contracts,contracts.test}.mjs`, `security/r016-evidence.schema.json`, `scripts/fixtures/supply-chain/{transitive-go.mod,transitive-go.sum}.txt`; shared governance/evidence yalnız orchestrator tarafından birleştirilir. Root Go manifestleri, runtime/compiler, image, workflow, policy thresholds ve scanner sürümleri kapsam dışıdır. Acceptance ve rollback: `state/W001-R016-GO-GRAPH.md`. Continuous first-parent path union ve manifest-only seal her yeni exact head'de doğrulanır; eski base protected-parity reddi bootstrap istisnası olarak açık kalır, trusted runtime PASS sayılamaz.
