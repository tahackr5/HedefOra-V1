...
- Bir sonraki living manifest seal; tarihsel `4cacb40 → b9e1ab5` JSON-only seal'ini, `b9e1ab5 → 659c1cb` remediation'ını, `659c1cb → 7a987f7` iki exact docs path'ini ve bu state/evidence promotion'ını ayrı kesintisiz ranges olarak kaydeder. Seal sonrası bütün exact-head gates baştan çalışır; eski `b9e1ab5` kanıtı yeni head'e taşınmaz.
