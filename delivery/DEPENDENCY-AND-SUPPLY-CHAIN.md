...
- R-016 yalnız GitHub repository ID `1349011765` ve full name `tahackr5/HedefOra-V1` birlikte eşleştiğinde owner-controlled internal CI amacıyla çalışır. Hosted event public/private görünürlüğü `github-event` authority ve `visibilityProof=true` ile; yerel koşum yalnız `local-declaration` ve `visibilityProof=false` ile kanıtlar. Fork/foreign head checkout, scanner image/rule fetch ve advisory DB acquisition başlamadan açık failure alır; skipped sonuç `PASS` değildir.
