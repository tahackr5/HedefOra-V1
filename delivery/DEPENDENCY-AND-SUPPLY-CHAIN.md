...
## Hosted güvenlik ve R-016 telafi kapısı

Hosted CodeQL/Dependency Review kullanılabiliyorsa exact target üzerinde ayrıca `PASS` vermelidir; koşmayan hosted kontrol yerel R-016 ile yeniden adlandırılmaz. W001 runtime davranışından önce:
...
- R-016 yalnız GitHub repository ID `1349011765` ve full name `tahackr5/HedefOra-V1` birlikte eşleştiğinde owner-controlled internal CI amacıyla çalışır. Hosted artifact, event bağlamını yalnız `github-context-claim` ve `visibilityProof=false`; yerel koşum yalnız `local-declaration` ve `visibilityProof=false` olarak kaydeder. Detached artifact hosted authority kanıtı değildir. Hosted `PASS` için authenticated GitHub API run/repository/event/workflow/head/job/artifact metadata'sı, GitHub artifact digest'i, indirilen archive digest'i ve internal evidence SHA-256/target seal'i dış verifier ile birebir bağlanır. Fork/foreign head checkout, scanner image/rule fetch ve advisory DB acquisition başlamadan açık failure alır; skipped sonuç `PASS` değildir.
- Normal `pull_request` workflow'u head/merge-ref tarafından değiştirilebilir olduğundan hiçbir repository-use veya lisans boundary'si uygulamaz ve kullanılmaz. `.github/workflows/ci.yml` yalnız owner repository içindeki `main` ve `codex/**` push'larında çalışır. PR enforcement yalnız default branch kontrollü `pull_request_target` workflow'undadır; checkout'suz source boundary fork/foreign head'i reddetmeden downstream job başlayamaz. Dependency Review bu workflow'da head checkout etmeden event SHA'larını GitHub Dependency Graph API ile karşılaştırır.
...
- Split-root koşumunda `.gitleaksignore`, `scripts/supply-chain/**`, `scripts/fixtures/supply-chain/**`, `tools/osvdbcheck/**`, R-016 security config'leri ve iki R-016 workflow'u base ile target arasında path, mode, stage ve Git object ID düzeyinde birebir aynı olmalıdır. Bu protected parity sağlanmadan config okunmaz veya scanner başlatılmaz; target'ın normal dependency/source değişiklikleri ise kendi Git blob kimlikleriyle taranır. `.gitleaksignore` yalnız doğrulanmış false-positive için exact commit:path:rule:line fingerprint'i taşıyabilir; rule, path, commit veya üç parçalı global istisna kabul edilmez. Exact byte set'i protected testte kilitlenir ve pinned Gitleaks sürüm/digest değişikliği fingerprint davranışının yeniden doğrulanmasını gerektirir.
...
- Public görünürlükte eski workflow tree'sine doğrudan revert yasaktır. Rollback sırası progression stop, owner-approved private geçiş, exact repository identity/visibility ve hosted capability yeniden doğrulaması, ardından reviewed revert'tir; source boundary private durumda da varsayılan olarak korunur.
...
- Resolved Go MVS graph'ı (main hariç bütün selected exact third-party modüller) ayrı deterministic `go-selected-modules-v1` scanner manifestine aktarılır. Original tracked go.mod/go.sum digest'leri değişmez; scanner manifestinin format/size/SHA kaydı ayrıdır. Original resolver staging ile scanner staging ayrılır, scan öncesi/sonrası byte seal yeniden doğrulanır. OSV Go extraction count'u synthetic manifestte third-party count'a, JSON package multiset'i seçili graph'a birebir eşleşmelidir; canonical `v` yalnız OSV package identity karşılaştırmasında bir kez kaldırılır, orijinal graph/pin hash'leri korunur. Bütün manifest-only OSV source çağrıları explicit `--no-call-analysis=go` taşır; `--all-vulns`, unknown scope/severity ve existing threshold değişmez, SAST/CodeQL ayrı gate'tir.
- Ek canlı transitive Go canary'nin tracked root manifesti yalnız pgx v5.10.0 gerektirir; root require içinde olmayan `golang.org/x/text v0.29.0` / `GO-2026-5970`, raw exit 1 ve selected 12-package exact multiset/extraction doğrulanır. Bu advisory'nin unknown severity'si normal production policy'de fail-closed kalır; yalnız doğru advisory ve tam coverage gözlemi canary PASS olur. Strict evidence hem fixture original blob'larını hem ayrı synthesized manifest ve MVS identity'lerini doğrular.
...
- OSV'nin npm JSON çıktısı dependency scope taşımadığından scanner kimliği `unknown` olarak korunur; kapsam uydurulmaz. Development canary'nin yalnız `devDependencies` üzerinden geldiği ayrı strict pnpm fixture parse'ıyla kanıtlanır ve evidence `fixtureScope=development`, `declaredDirectScopes=[devDependencies]`, `scannerScope=unknown`, `scannerScopeReported=false` değerlerini ayrı ayrı mühürler. Bütün scope'lar aynı CVSS eşiğiyle tarandığı ve `unknown` da bloklandığı için bu eşleme kapsamı gevşetmez. Eski `expectedScope` alanı ilk geçerli R-016 PASS artifact'ı oluşmadan önce düzeltilen yayımlanmamış sözleşmedir; eski biçimdeki artifact PASS olarak kabul edilmez ve migration penceresi yoktur.
...
- SAST, digest-pinned nonroot Semgrep Community Edition image'ını yalnız `--oss-only`, exact locked Git commit:path blob'larından run-specific staging'e alınan rule files, `--network none`, read-only mounts, kapalı metrics/version-check ve bounded resource/timeout ile çalıştırır. Tüm tracked Go ve JavaScript/TypeScript regular source dosyaları checkout byte'larına güvenmeden exact Git index OID/blob'larından ayrı staging'e kopyalanır; ignore, binary ve 1 MB target filtreleri desteklenen seçeneklerle kapatılır. Minified exclusion etkinleştirilmez; pinned Semgrep 1.175.0 varsayılan olarak minified dosyaları tarar. `paths.scanned` exact Git source multiset'iyle birebir eşleşmezse koşum reddedilir. Bilerek insecure Go/TypeScript, `.semgrepignore` altında yer alan insecure TypeScript ve hem 1 MB üstü hem minified olan bypass fixture'larının finding üretmemesi de gate failure'dır; `.semgrepignore` dosyasının kendisi de exact scan path parity'sine dahildir ancak finding beklenmez.
- Yerel CLI, açıkça verilmiş absolute `--git-binary` ve `--docker-binary` yolları ile public/private görünürlük dahil exact `R016_*` local-declaration bağlamı olmadan başlamaz; bu beyan hosted görünürlük kanıtı değildir. Dosyaların regular/non-link, executable, target/control/temp kökleri dışında ve hash-mühürlü olması gerekir. Hosted Linux koşumu yalnız `/usr/bin/git`, `/usr/bin/docker` ve trusted toolcache içindeki exact Node `24.20.0` ile çalışır. Git komutları exact repository `-C` bağıyla, system/global config ve routing/trace environment kapalı; Docker komutları run-specific boş config ve routing/executable-injection environment kapalı çalışır.
...
- Semgrep Rules License v1.0 kapsamındaki kurallar yalnız exact first-party HedefOra repository'sinin owner-controlled internal CI taramasında geçici olarak alınır. Public/private görünürlük kullanım sınırını değiştirmez; rule içeriği artifact/repository'ye vendored edilmez, dış repository/fork taranmaz ve scanning-as-a-service sunulmaz. Repository rename/transfer, kimlik değişimi veya farklı kullanım amacı yeni owner/security review olmadan fail-closed kalır.
