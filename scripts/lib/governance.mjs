...
export function readTomlString(document, key) {
  ...
  const match = document.match(new RegExp(`^${escapedKey}\\s*=\\s*"([^"]+)"\\s*$`, "m"));
  return match?.[1] ?? null;
}
...
export async function validateGovernance(repositoryRoot) {
  ...
  for (const file of agentFiles) {
    ...
    if (hasTomlKey(document, "model") || hasTomlKey(document, "model_reasoning_effort")) {
  ...
  for (const reviewer of ["security-privacy-review.toml", "cold-reviewer.toml"]) {
    const document = await readFile(path.join(agentDirectory, reviewer), "utf8");
  ...
  const quality = await readFile(path.join(agentDirectory, "quality-testing.toml"), "utf8");
  if (hasTomlKey(quality, "sandbox_mode")) {
    errors.push("quality-testing.toml: sandbox_mode must inherit its task mode");
  }

  const config = await readFile(path.join(repositoryRoot, ".codex", "config.toml"), "utf8");
  ...
  for (const directory of skillDirectories) {
    const document = await readFile(path.join(skillsDirectory, directory, "SKILL.md"), "utf8");
    const metadata = parseFrontmatter(document);
    if (!metadata?.name || !metadata?.description) {
      errors.push(`${directory}/SKILL.md: missing name or description frontmatter`);
      continue;
    }
  ...
  if (JSON.stringify(skillNames.sort()) !== JSON.stringify(expectedSkillNames)) {
    errors.push(`skill names differ: ${skillNames.sort().join(", ")}`);
  }

  const fixtures = JSON.parse(
    await readFile(path.join(repositoryRoot, "scripts", "fixtures", "skill-prompt-cases.json"), "utf8"),
  );
  for (const fixture of fixtures) {
    ...
    if (!expectedSkillNames.includes(fixture.expectedSkill)) {
      errors.push(`${fixture.id}: unknown expected skill ${fixture.expectedSkill}`);
    }
  }
